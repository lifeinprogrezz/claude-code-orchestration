...
## From the official source (asd-ste100.org + STEMG White Paper, crawled 2026-08-17, local fork)

Full site digest: [references/asd-ste100-site-digest.md](references/asd-ste100-site-digest.md). What the official source adds to the kit:

- **Technical nouns and technical verbs are the open-vocabulary escape hatch.** Domain terms from official documentation, glossaries, or term bases are permitted even when the word is not in the dictionary. Keep a project glossary and treat its terms as approved. Use this path sparingly.
- **Rules over dictionary.** The most common failure of untrained writers is to police words and neglect the writing rules. The sentence and structure rules do the work.
- **"-ing" exceptions**: the dictionary approves some -ing words as other parts of speech — opening (n), remaining (adj), something (pron), during (prep). Inside a technical noun, an -ing form is acceptable.
- **Conditions first** is official phrasing: "If hot oil touches your skin, injuries can occur."
- **Reader-first asymmetry**: STE is easy to read and demanding to write (C1 English recommended for writers; readers need zero training). It is not "baby English".
- **Spoken use**: the rules and the dictionary also support clearer meetings and presentations — apply the discipline to speaker notes and live delivery.
- **Translation benefit**: controlled source text translates better, by humans or by LLMs. Useful for EN/ES bilingual work.
- **AI position (STEMG White Paper, June 2026)**: AI must support, never override, the standard. Accountability stays with the human author. Declare AI-assisted content where it matters.
- **Evidence, cited properly**: Chervak, Drury & Ouellette (1996), "Simplified English for Aircraft Workcards", HFES 40th Annual Meeting, DOI 10.1177/154193129604000502 — 175 aircraft maintenance technicians; comprehension significantly improved with Simplified English, most for difficult material and non-native speakers; layout had no effect.
- Maintenance: Issue 9 (Jan 2025) is current. **Issue 10 is scheduled for January 2028** — re-check this skill then. The full standard is free on request (PDF via form at asd-ste100.org).

## Spanish adaptation (added 2026-08-17, local fork)

Same system, Spanish surface. Apply when the deliverable is in Spanish (client work). The linter is English-only; for Spanish, use this checklist manually:

- Same caps: max 20 words for an instruction sentence, 25 for descriptive text. One idea per sentence. One topic per paragraph, max 6 sentences.
- Active voice and simple tenses. Use a verb for the action: "analizar el log", not "llevar a cabo un análisis del log". realizar/efectuar → hacer.
- Short common words: utilizar→usar · previamente→antes · posteriormente→después · sin embargo / no obstante→pero · por consiguiente / por lo tanto→así que · requerir→necesitar · disponer de→tener · finalizar→terminar.
- Delete filler openers: "cabe destacar que", "es importante señalar que", "en este sentido".
- One term, one meaning: do not rotate synonyms for the same thing (boutique / tienda / punto de venta — pick one and repeat it).
- No marketing adjectives: integral, robusto, puntero, revolucionario, sin fricciones, de primer nivel.
- No semicolons. In strict mode, no em dashes (—).
- Do not drop articles to compress. No colloquial contractions.
